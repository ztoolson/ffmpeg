...
I<This filter is deprecated. Use> B<aformat> I<instead.>
...
with the timestamp I<start> will be the first sample in the output.
...
B<start>, B<end>, B<duration> are expressed as time
...
B<attacks> refers to increase of volume and B<decays> refers
...
Typical value for attack is C<0.3> seconds and for decay C<0.8>
seconds.
...
The point C<0/0> is assumed but may be overridden (by C<0/out-dBn>).
Typical values for the transfer function are C<-70/-70 -60/-20>.


=item B<soft-knee>

Set amount for which the points at where adjacent line segments on the
transfer function meet will be rounded. Defaults is C<0.01>.
...
initially quiet is -90 dB. Default is C<0>.


=item B<delay>

Set delay in seconds. Default is C<0>. The input audio
...
C<av_parse_time()> for the accepted format, also check the "Time duration"
...
C<16x16>. Since the filter can be slow, you may want to reduce this value,
...
section in the ffmpeg-utils manual), a position (specified by the syntax I<X>/I<Y>,
...
input display aspect ratio. Calculated from C<(iw / ih) * sar>.
...
Same as I<start>, except this option sets the start timestamp in timebase
units instead of seconds.


=item B<end_pts>

Same as I<end>, except this option sets the end timestamp in timebase units
instead of seconds.
...
how to set the parameters read the section B<frei0r> in the description of
the video filters.
